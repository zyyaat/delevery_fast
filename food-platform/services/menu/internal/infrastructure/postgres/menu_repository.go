...
func (r *CategoryRepository) FindByRestaurant(ctx context.Context, restaurantID uuid.UUID) ([]*domain.Category, error) {
        ...
        for rows.Next() {
                ...
                if err := rows.Scan(&id, &restaurantID, &name, &displayOrder); err != nil {
                        return nil, err
                }
                cats = append(cats, domain.ReconstructCategory(id, restaurantID, name, displayOrder))
        }
...
func scanMenuItem(s scanner) (*domain.MenuItem, error) {
        ...
        return domain.ReconstructMenuItem(
                id, restaurantID, categoryID, name, description,
                price, imageURL, isAvailable, prepTimeMin,
                rating, ratingCount, isMostOrdered, displayOrder,
